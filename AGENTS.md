...
- Update the demo catalog, documentation, and capability/upstream contract in
  the same step as a demo.
- Do not modify `../sw-mlpl` during downstream work. Record missing general
  capabilities here and request separately authorized upstream work.
...
- Stage named files, not broad worktree sweeps. Preserve unrelated user work.

## Demo and security rules

- Keep demos deterministic, standalone, self-checking, and honest about which
  work is performed in MLPL, a native primitive, or an external tool.
- State memory complexity and measure a high-water bound for every demo that
  claims bounded-memory behavior.
- Treat checkpoint/pickle inputs as untrusted programs. Parse an allow-listed
  tensor-only subset; never invoke Python, arbitrary globals, constructors,
  reducers, extension opcodes, or external deserialization implicitly.
- Refuse unsupported executable/object constructs and report them. Do not call
  weights themselves unsafe; describe the risk as executable serialization.
- Conversion must verify tensor names, shapes, dtypes, byte ranges/hashes, and
  output-format invariants before reporting success.
- Exercise empty, truncated, malformed, adversarial-size, overflow, alignment,
  duplicate-name, unsupported-dtype, and boundary cases where applicable.
- Never claim large-file support while an implementation uses whole-file
  `read_bytes`; distinguish bounded range I/O from browser/server streaming.
- 3D clients consume derived, hierarchical visualization data, not entire
  model files. Level-of-detail requests must remain bounded.

## Planning references

- Research transcript: `docs/sw-mlpl-demo-ml-utils-discussion.txt`
- Delivery plan and capability gates: `docs/plan.md`
- Active/future saga queue: `docs/sagas.md`
- Neighbor repository audit: `docs/peer-repository-audit.md`
