...
**11**(10):159, 2018, Section 2.1, Equation (1), verified against the paper:

$$ y_{r,x,y} = \sum_{q=1}^{Q} \sum_{u=1}^{M_w} \sum_{v=1}^{N_w}
   w_{r,q,u,v}\; x_{q,x+u,y+v} $$

Every rung displays its equation under [the math notation
rules](math-notation.md): explicit summation limits, a symbol table, the
1-based-to-0-based translation stated, and cross-correlation named. Equation (1)
contains no bias and no activation, so rung 5 must say that it is going beyond
what the paper's definition states.
...
None of this is a claim to exclusivity. Convolution is fundamental enough that
several repositories should show it, each for its own audience; the positioning
below says why this one earns its version, not why others should not have one.
...
Trainable convolution, unlocked by the upstream `autograd(windows)` work, stays
outside this saga: it owes finite-difference gradient checks, a training
trajectory, and a stopping rule, so it is its own saga gated on this forward
ladder being accepted.

Upstream's own convolution demos are not a conflict. Both repositories build
them on purpose — the live demos and literate docs show the primitives earning
their place, and `demos/cnn/` here shows what the tensors in a model artifact
compute. The full ladder is built here regardless of what exists upstream. What
must not drift is the shared arithmetic: the same paper, the same hand-checked
-6 edge-filter cell, the same `conv2d` oracle.
